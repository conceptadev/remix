...
known Radix Themes 3.3.0 color values. `open_code/agent_fixture/` is a third,
much smaller consumer used by `tool/check_agent_consumer.dart`; see
...
It installs four default-preset items with the default `Ui` prefix, points
`remix` and `remix_agent` at this checkout, and runs the fixture's suite. It
then edits the installed IconButton recipe and reruns the suite with the new
expected metric, which is what proves Agent's send and stop buttons are painted
by the application's file. It has no hosted phase, because `remix_agent` has no
