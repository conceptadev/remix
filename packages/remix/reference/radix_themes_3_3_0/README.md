...
boundaries, Atlas inventory, and reference-fixture paths live in
`manifest.json`.
...
beta.5 dependency, or an undocumented approximation. Parameterized cases use
...
Chromium reference output is kept under `chromium/`; Flutter Atlas output is
kept separately at the workspace level so browser and engine rasterization are
never used as self-validating oracles.
