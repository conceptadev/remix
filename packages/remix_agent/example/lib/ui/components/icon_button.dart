...
/// Geometry for one [UiIconButtonSize].
typedef _UiIconButtonMetrics = ({double edge, double iconSize});

_UiIconButtonMetrics _metricsFor(UiIconButtonSize size) => switch (size) {
...
IconButtonStyler _base(_UiIconButtonMetrics metrics) => IconButtonStyler()
...
IconButtonStyler _variantStyle(UiIconButtonVariant variant) =>
    switch (variant) {
      .primary => _filled(
...
IconButtonStyler _quiet({required bool bordered}) {
  ...
  if (bordered) {
    style = style.border(.color(UiTokens.border()).width(_borderWidth));
  }
