...
class CarbonMultiselect<T extends Object> extends StatelessWidget {
  ...
  List<CarbonMultiselectItem<T>> _selectedItems() =>
      .unmodifiable(items.where((item) => selectedValues.contains(item.value)));

  String _selectionLabel(List<CarbonMultiselectItem<T>> selected) {
    if (selected.isEmpty) return placeholder;

    return selectionLabelBuilder?.call(selected) ??
        selected.map((item) => item.label).join(', ');
  }

  MenuStyler _menuStyle({required bool hasSelection, required double height}) {
    final borderColor = errorText == null
        ? _carbonMultiselectBorder()
        : CarbonTokens.supportError();

    return carbonMenuStyle(size: carbonMenuSizeFor(size)).trigger(
      MenuTriggerStyler()
          .width(.infinity)
          .height(height)
          .mainAxisSize(.max)
  ...
  Widget build(BuildContext context) {
    ...
    final menu = _menu(
      triggerLabel: _selectionLabel(selected),
