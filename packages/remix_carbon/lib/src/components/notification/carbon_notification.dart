...
extension on CarbonNotificationKind {
  ColorToken get statusColor => switch (this) {
  ...
  ColorToken get lowContrastBackground => switch (this) {
  ...
  SemanticsRole get semanticsRole => switch (this) {
    .warning || .error => .alert,
    .info || .success => .status,
  };
}
...
class CarbonNotification extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    var calloutStyle = CalloutStyler()
        ...
        .color(background())
        .border(
          BoxBorderMix.left(BorderSideMix(color: kind.statusColor(), width: 3)),
        );
    if (variant == .toast) {
          ...
          children: [
            ExcludeSemantics(
              child: Icon(
                ...
                color: kind.statusColor.resolve(context),
              ),
            ),
