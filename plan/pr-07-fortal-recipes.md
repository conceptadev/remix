...
> Theme Skeleton, SegmentedControl, TextArea, and DataList from the pinned Radix 3.3 source; finish menu indicators; document CheckboxGroup composition; and advance the executable parity ledger atomically.

## PR contract

- Title: `feat(fortal): add recipes for new components`
- Depends on: PRs 1-6 merged.
- Compatibility: additive generated wrappers/enums/tokens plus internal TextField recipe refactoring; no migration.
- Primary outcome: four new mapped Fortal families, exact menu item indicators, and a documented CheckboxGroup composition using `FortalCheckbox` visuals.
- Out of scope: a styled `FortalCheckboxGroup`, typography (PR 8), SegmentedControl sliding-indicator behavior, browser TextArea resizing, DataList responsive prop DSL/leading trim, and unrelated component gaps.
...
| `textAreaMinHeight3` | `DoubleToken`, 80 × scaling | Size 3; sizes 1/2 reuse `space8`/`space9`. |
| `textAreaPaddingY1` | `DoubleToken`, `4 × scaling - borderWidth1` | Bordered size-1 vertical inset. |
| `dataListRowGap3` | `DoubleToken`, 20 × scaling | Radix `space4 × 1.25`. |
| `dataListLabelMinWidth` | `DoubleToken`, 120 × scaling | Horizontal implicit label minimum. |

Reuse existing `textFieldPadding1/2/3`, `selectSpace1Half`, space, radius, text, gray/accent, focus, and shadow tokens for every other metric. Add light/dark/scaling resolution coverage to `fortal_tokens_test.dart` / `fortal_theme_resolution_test.dart`. Do not introduce a token when a current one is exact.
...
- inactive label regular, active label medium; disabled text `grayA8`; unselected hover `grayA2`;
- selected background `segmentedControlIndicatorBackground`;
- surface selected ring `grayA4`; classic selected `shadow2Layers`;
- disabled selected background `grayA3` with no selected shadow;
- focus ring painted through `foregroundDecoration` at 2 px and -1 inset so layout never moves.
...
- classic/surface/soft fill, text, placeholder, disabled/read-only, error, focus ring, cursor, and effect layers reuse refactored private TextField helpers.

Refactor `fortal_textfield_styles.dart` into shared private “apply classic/surface/soft/neutral/disabled/error effects to a supplied base styler” helpers. Preserve every existing TextField resolved-style test before applying the TextArea base metrics. Do not copy the three complete variant bodies.

Record browser `resize` as an approximation: Flutter uses `minLines`, `maxLines`, constraints, and scrolling with no web drag handle. Responsive CSS size/radius props map to rebuilt Flutter values/scope, as elsewhere.
...
Expose global `highContrast` for the data-driven list because per-label recipe props do not exist. Manifest-defer per-label color/highContrast/width props, responsive breakpoints, and CSS leading trim with a clear Flutter alternative/tolerance. Do not silently claim them.

### Menu and CheckboxGroup

- Extend `_fortalMenuItemStyler` to style the new `indicator` slot in idle/highlighted/disabled states using the same size metrics and foreground roles as leading/trailing icons.
- Verify checked/unchecked check/dot and directional submenu chevron in solid/soft, size1/2, disabled, highContrast, light/dark, LTR/RTL.
- Remove PR 1's temporary neutral-indicator approximation and update menu evidence/selectors/states if needed.
- Add a Fortal section to CheckboxGroup docs showing `RemixCheckboxGroupItem(style: fortalCheckboxStyle(...))`. Do not create `FortalCheckboxGroup` or `FortalCheckboxGroupItem`; the group has no visual anatomy.
...
- [ ] Task 1: Add failing token, enum/default, wrapper, and resolved-metric tests.
  ...
  - Acceptance: all six new tokens, four wrappers, enum domains/defaults, named constructors, generic inference, and scale/light/dark values are specified before recipes exist.

- [ ] Task 2: Implement tokens and the Skeleton/Segmented recipes.
  - Files: `fortal_theme.dart`; new Fortal style parts in skeleton/segmented libraries; library part declarations; generated files; per-family parity tests.
...
- [ ] Task 3: Refactor TextField recipe helpers and add TextArea.
  ...
  - Acceptance: before/after resolved TextField styles are identical for every size/variant/state; TextArea changes only base multiline metrics.
...
- [ ] Task 4: Implement DataList and finish Menu/CheckboxGroup presentation.
  - Files: new DataList Fortal part/generated file, `menu/fortal_menu_styles.dart`, menu parity tests, CheckboxGroup docs/playground.
  - Acceptance: four recipes and menu indicator cover light/dark/high-contrast/state matrices with no new checkbox-group visual type.

- [ ] Task 5: Advance the parity contract from 20/3 to 24/3.
  - Files: `reference/.../manifest.json`, `coverage_evidence.json`, `tool/fortal_parity/check.dart`, per-family parity tests, reference README.
  ...
  - Acceptance: evidence owners are exactly `theme + 24 mapped + 3 extensions`, and every cited case string exists in a real test.
...
- Segmented selected/unselected/hover/pressed/focus/disabled/RTL states style without changing selection semantics or geometry.
- TextArea idle/hover/focus/disabled/readOnly/error and multiline semantics match TextField behavior.
...
- Inspect focus at 200% zoom and ensure foreground rings do not shift content.
- Verify selected Segment surfaces, all TextArea variants/sizes, DataList column geometry, Skeleton pulse/reduced motion, and menu indicators.
...
- [ ] All styling resolves from Fortal tokens; the six justified new tokens have theme/scaling tests.
- [ ] Existing FortalTextField output is unchanged by shared-helper refactoring.
- [ ] Menu indicators are fully Fortal-styled and the temporary approximation is removed.
- [ ] CheckboxGroup remains a nonvisual Remix coordinator composed with the existing Fortal checkbox recipe.
- [ ] Manifest/evidence/checker/tests/fixture all agree on 24 mapped + 3 extensions = 27 families.
- [ ] Segmented slide/separators, TextArea resize, and DataList per-label/responsive/trim gaps are explicit approximations.
...
- Risk: TextField helper refactor changes existing pixels. Mitigation: snapshot resolved specs in existing size/variant/state tests before refactoring and require equality.
