...
class VanillaLineChart extends StatelessWidget {
  const VanillaLineChart({
    super.key,
...
///
/// [style] merges last, so one call site can replace any part of the recipe.
class VanillaBarChart extends StatelessWidget {
  const VanillaBarChart({
    super.key,
    this.palette,
  ...
  Widget build(BuildContext context) {
    return BarChart(
      key: this.key,
      style: vanillaBarChartStyle(palette: this.palette, style: this.style),
      groups: this.groups,
      xAxis: this.xAxis,
      yAxis: this.yAxis,
      topAxis: this.topAxis,
