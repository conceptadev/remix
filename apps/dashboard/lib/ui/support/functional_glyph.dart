...
///
/// Every collapsible Ui surface offers the host the same escape hatch — a
...
class UiDisclosureIndicator extends StatelessWidget {
  const UiDisclosureIndicator({
    super.key,
...
/// Internal Material-free glyph set used by Ui's functional defaults.
///
...
class UiFunctionalGlyph extends StatelessWidget {
  const UiFunctionalGlyph({
    super.key,
    required this.kind,
    required this.spec,
    this.expanded = false,
  ...
  final UiFunctionalGlyphKind kind;
  final IconSpec spec;
  final bool expanded;
  ...
  Widget build(BuildContext context) {
    ...
    return ExcludeSemantics(
      child: kind == UiFunctionalGlyphKind.loading
