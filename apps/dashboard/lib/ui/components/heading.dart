...
/// Ui-themed heading style on the Radix nine-step scale.
///
...
/// renders the styler, and [UiHeading] must additionally publish a native
/// heading node that generation cannot supply.
TextStyler uiHeadingStyle({
  UiTextSize size = .size6,
  UiTextWeight weight = .bold,
  TextAlign? align,
  bool softWrap = true,
  bool truncate = false,
  bool accent = false,
  ...
  var recipe = TextStyler(
    style: uiTextSizeToken(size).mix(),
  ).height(lineHeight).fontWeight(uiTextWeightToken(weight)());
  // Neutral headings pin `gray12` from the tokens rather than inheriting the
  // ambient foreground, matching uiTextStyle's token-default contract.
  recipe = accent
...
class UiHeading extends StatelessWidget {
  const UiHeading(
    ...
    super.key,
    this.headingLevel = 1,
    this.size = UiTextSize.size6,
    this.weight = UiTextWeight.bold,
    this.align,
    this.softWrap = true,
    this.truncate = false,
    this.accent = false,
