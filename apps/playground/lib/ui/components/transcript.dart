...
class PlaygroundTranscript extends StatefulWidget {
  const PlaygroundTranscript({
    super.key,
    ...
    this.style = const PlaygroundTranscriptStyler.create(),
    this.styleSpec,
  }) : itemCount = null,
       itemBuilder = null;

  const PlaygroundTranscript.builder({
    super.key,
  ...
  final PlaygroundTranscriptSpec? styleSpec;

  @override
  State<PlaygroundTranscript> createState() => _PlaygroundTranscriptState();
}

class _PlaygroundTranscriptState extends State<PlaygroundTranscript> {
  ScrollController? _ownedController;
  late ScrollController _controller;
  late final PlaygroundLiveEdgeEngine _liveEdge;
  ...
  /// [PlaygroundTranscriptSpec.viewport] has never had an effect. Passing a
  ...
  void initState() {
    ...
    _liveEdge = PlaygroundLiveEdgeEngine(
  ...
  @override
  void didUpdateWidget(PlaygroundTranscript oldWidget) {
  ...
  Widget build(BuildContext context) {
    return RemixStyleSpecBuilder<PlaygroundTranscriptSpec>(
      style: widget.style,
  ...
  Widget _buildList(PlaygroundTranscriptSpec spec, {required bool shrinkWrap}) {
    final children = widget.children;
    final count = children?.length ?? widget.itemCount!;
    final spacing = spec.spacing ?? 0;
    assert(spacing >= 0, 'PlaygroundTranscript spacing must be non-negative.');
...
@MixableSpec(target: PlaygroundTranscript.new)
@immutable
final class PlaygroundTranscriptSpec with _$PlaygroundTranscriptSpec {
  @override
  ...
  const PlaygroundTranscriptSpec({
