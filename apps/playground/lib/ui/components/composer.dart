...
class PlaygroundComposer extends StatefulWidget {
  const PlaygroundComposer({
    ...
    this.style = const PlaygroundComposerStyler.create(),
    this.styleSpec,
  }) : assert(
  ...
  final PlaygroundComposerSpec? styleSpec;
...
class _PlaygroundComposerState extends State<PlaygroundComposer> {
  ...
  @override
  void didUpdateWidget(PlaygroundComposer oldWidget) {
  ...
  Widget _defaultSubmitIcon(
    ...
    IconSpec spec,
    IconData? icon,
  ) => PlaygroundFunctionalGlyph(kind: .send, spec: spec);

  Widget _defaultStopIcon(
    BuildContext context,
    IconSpec spec,
    IconData? icon,
  ) => PlaygroundFunctionalGlyph(kind: .stop, spec: spec);

  @override
  Widget build(BuildContext context) {
    return RemixStyleSpecBuilder<PlaygroundComposerSpec>(
      style: widget.style,
                  ...
                  children: [
                    ...
                    Semantics(
                      ...
                      child: RemixIconButton(
                        key: ValueKey(
                          widget.running
                              ? 'playground-composer-stop'
                              : 'playground-composer-send',
                        ),
                        icon: null,
...
@MixableSpec(target: PlaygroundComposer.new)
@immutable
final class PlaygroundComposerSpec with _$PlaygroundComposerSpec {
  @override
  final StyleSpec<FlexBoxSpec> toolbar;

  const PlaygroundComposerSpec({StyleSpec<FlexBoxSpec>? toolbar})
