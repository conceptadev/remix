...
DataListStyler playgroundDataListStyle({
  DataListStyler style = const DataListStyler.create(),
}) => DataListStyler()
    .label(.fontSize(_textSize).color(PlaygroundTokens.mutedForeground()))
    .value(.fontSize(_textSize).color(PlaygroundTokens.foreground()))
    .rowSpacing(_rowSpacing)
    .columnSpacing(_columnSpacing)
    .labelValueSpacing(_labelValueSpacing)
...
/// Text size for both roles, matching body copy.
const _textSize = 14.0;

/// Gap between one pair and the next.
const _rowSpacing = 12.0;

/// Gap between two pairs sitting side by side.
const _columnSpacing = 24.0;

/// Gap between a label and its own value.
const _labelValueSpacing = 8.0;
