...
class PlaygroundDisclosureIndicator extends StatelessWidget {
  const PlaygroundDisclosureIndicator({
    super.key,
...
enum PlaygroundFunctionalGlyphKind {
  send,
  stop,
  copy,
  retry,
...
class PlaygroundFunctionalGlyph extends StatelessWidget {
  const PlaygroundFunctionalGlyph({
    super.key,
    required this.kind,
    required this.spec,
    this.expanded = false,
  ...
  final PlaygroundFunctionalGlyphKind kind;
  final IconSpec spec;
  final bool expanded;
  ...
  Widget build(BuildContext context) {
    ...
    return ExcludeSemantics(
      child: kind == PlaygroundFunctionalGlyphKind.loading
