...
stop, no link role, no enabled state, and no activation. Give it `onPressed` and
it becomes a real link — focusable, activatable with pointer and Enter, and
underlined according to `underline`.
