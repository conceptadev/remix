...
That `1em` is defined by `FortalScope`, which installs the Radix theme root's
text run — `text3` (16px, 1.5 line height) at `gray-12`, regular weight. So an
unsized `FortalCode` resolves `0.95 × 0.95 × 16px`, exactly as upstream does.
Under `MaterialApp` or `CupertinoApp` the scope must sit in `builder:` for that
to hold; see [scope placement](/fortal#scope-placement).
