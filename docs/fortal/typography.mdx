...
For visual reference, open the [light capture](/assets/fortal-typography-light.png)
or [dark capture](/assets/fortal-typography-dark.png). These are screenshots,
not interactive previews.
...
`DefaultTextStyle` for bare Flutter `Text`. Under `WidgetsApp`, put the scope in `builder:` so that fallback reaches routes and
