...
## Contextual shorthand convention

Use contextual dot shorthand for nested stylers, where the receiving API makes
the expected type clear:

- Use named factories for state variants: `.onHovered(.color(value))`.
- Use `.new()` only when a nested styler starts with a lifecycle operation
  that has no named factory, such as `.onPressed(.new().wrap(...))`.
- Use shorthand for nested component slots such as `label`, `icon`,
  `container`, `content`, and `item`.

Keep the concrete class name for root styles, standalone variables, widget
`style:` arguments, and constructor defaults. This makes the primary styled
component explicit while keeping its nested pieces concise.
...
The menu styler owns `trigger`, popup `overlay`, `containerEffects`, `item`,
`label`, and `divider` slots. The `trigger` slot styles the established
`RemixMenuTrigger`; arbitrary trigger widgets remain caller-styled.

`RemixSelectStyler` retains the established `menuContainer` FlexBox slot and
adds the `content` surface plus item, label, and separator slots. Style the
popup layout through `menuContainer`; style its painted surface through
`content`.
...
for example `RemixCardStyler.container(.new(transform: matrix,
...
behavior remain unchanged. The source-authored wrappers mirror `@MixWidget`'s
variant-constructor convention: use `FortalButton.ghost(...)` when the variant
is known and retain `FortalButton(variant: currentVariant, ...)` for runtime
selection. The widget builder can be re-enabled once upstream generation
supports that dependency ordering.
