...
Here, `backgroundColor` aliases the canonical `color` operation, and
`.padding(.all(12))` calls `padding` directly. Conveniences and aliases do not
add duplicate named factories.
...
| `AvatarStyler.square(value)` | `.size(value, value)` |
| `AvatarStyler.foregroundColor(value)` | `.label(.color(value)).iconColor(value)` |
| `BadgeStyler.foregroundColor(value)` | `.label(.color(value))` |
| `CalloutStyler.foregroundColor(value)` | `.icon(.color(value)).textColor(value)` |
