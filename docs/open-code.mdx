...
| Item | Installs | Widgets you get |
|---|---|---|
| `theme` | `tokens.dart`, `theme_data.dart`, `theme_scope.dart` | `UiTokens`, `UiThemeData`, `UiThemeScope` |
| `button` | `components/button.dart` + generated adapter | `UiButton` |
| `checkbox` | `components/checkbox.dart` + generated adapter | `UiCheckbox`, `UiCheckboxGroupItem` |
| `tabs` | `components/tabs.dart` + generated adapter | `UiTabBar`, `UiTab`, `UiTabView` |

One `add` installs one item. The names above use the default `Ui` prefix;
`--prefix Acme` gives you `AcmeButton` and friends.

Some Remix widgets are behavioral and carry no style, so the registry has
nothing to render for them. `RemixCheckboxGroup` and `RemixTabs` are the two
in this catalog: import them from `package:remix/remix.dart` and put the
installed adapters inside.
