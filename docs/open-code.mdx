...
`components/<item>.dart`, and depends on `theme`. One item depends on more:
`data_table` also pulls in `checkbox`, `icon_button`, and `select`, because its
selection column, pager, and page-size control *are* those components — adding
it installs all four.
