...
```bash
remix init --prefix Acme --preset default --ui-path lib/design_system
```
...
| `preset` | `default` | Bundled registry selected for every later `add`. It must be chosen at `init`; this release bundles `default`. |
