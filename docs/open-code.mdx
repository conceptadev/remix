...
and a collapsed rail labels it with a tooltip; and `sidebar_layout` is a
layout, not a styled component — it has no `Spec` and no generated adapter,
and it pulls in `sidebar` to compose into its row and compact sheet.

The registry requires Remix beta.9, which includes `RemixSidebar`.
