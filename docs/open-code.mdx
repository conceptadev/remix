...
```bash
# Project-local dev dependency (recommended, and the only option today)
dart run remix_cli:remix <command>

# Global install — available once remix_cli is published to pub.dev
dart pub global activate remix_cli
remix <command>
```

Prefer the project-local form even after publishing: it pins the CLI *and* its
bundled registry snapshot per project, so two apps on different Remix versions
cannot install each other's templates.
...
The bundled registry is deliberately small, and every component depends on
`theme`:

`theme` installs `tokens.dart`, `theme_data.dart`, and `theme_scope.dart`,
giving you `UiTokens`, `UiThemeData`, and `UiThemeScope`. Every other item
installs one authored file plus its generated adapter under
`components/<item>.dart`, and depends on `theme`:

| Item | Widgets you get | Axes |
|---|---|---|
| `avatar` | `UiAvatar` | size |
| `badge` | `UiBadge` | variant |
| `button` | `UiButton` | variant, size |
| `callout` | `UiCallout` | variant |
| `card` | `UiCard` | — |
| `checkbox` | `UiCheckbox`, `UiCheckboxGroupItem` | size |
| `divider` | `UiDivider` | orientation |
| `icon_button` | `UiIconButton` | variant, size |
| `link` | `UiLink` | — |
| `progress` | `UiProgress` | size |
| `radio` | `UiRadio` | size |
| `segmented_control` | `UiSegmentedControl` | size |
| `skeleton` | `UiSkeleton` | — |
| `slider` | `UiSlider` | size |
| `spinner` | `UiSpinner` | size |
| `switch` | `UiSwitch` | size |
| `tabs` | `UiTabBar`, `UiTab`, `UiTabView` | size |
| `textfield` | `UiTextField`, `UiTextArea` | size |
...
nothing to render for them. `RemixCheckboxGroup`, `RemixRadioGroup`, and
`RemixTabs` are the three in this catalog: import them from
`package:remix/remix.dart` and put the installed adapters inside.

`toggle_group` and `segmented_control` work the other way round. Their options
are data, not widgets, and the group's own recipe carries the option style — so
one `add` covers the container *and* every option, and an option in a loop
cannot be left unstyled.
