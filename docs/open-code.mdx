...
`components/<item>.dart`. Four items differ: `icons` installs app-owned
...
and page-size control *are* those components — adding it installs all four; and
`sidebar` pulls in `toggle`, because a destination *is* one.
