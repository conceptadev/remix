...
For a guided walkthrough with screenshots and runnable sample projects, start
with [Build a settings screen](/tutorials/settings-screen). This page is the
...
Until Remix beta.10 is published, add this temporary `pubspec_overrides.yaml`
...
[complete routed host example](/fortal#scope-placement).

The application owns the appearance preference: light, dark, or system. Resolve
system appearance from `MediaQuery.platformBrightnessOf(context)` inside the
app builder so platform changes rebuild the theme. An explicit light or dark
preference takes precedence over the system value.

The installed presets currently expose different theme APIs:

| Preset | Supply the resolved appearance |
| --- | --- |
| `default` | Pass `AcmeThemeData.light()` or `AcmeThemeData.dark()` to `AcmeThemeScope(data: ...)` |
| `fortal` | Pass the resolved `Brightness` to `AcmeScope(brightness: ...)`; keep accent, gray, radius, and scaling consistent across modes |

Use the configured prefix in place of `Acme`. The CLI installs the theme layer;
it does not currently generate a complete application host or appearance picker.
The [downloadable tutorial apps](/tutorials/settings-screen) demonstrate manual
light/dark switching with both presets.

Generated scopes take a `child`. `WidgetsApp.builder` handles placement; it does
not need a matching builder constructor on the scope. If a widget must read the
newly installed theme while building, put a Flutter `Builder` below the scope
and use that callback's context. The app builder's own context is above the
scope it returns.

The reference starter contract is to preserve the selected theme across routes,
update visible overlays when appearance changes, support system appearance
changes, and preserve application-owned theme edits during regeneration. Theme
selection and persistence belong to the application; component recipes consume
the resolved theme. A shared `RemixScope` or theme base class is not currently
part of the generated API.
...
```
Resolved remix 1.0.0; this remix_cli registry was authored against 1.0.0-beta.10. Run flutter pub upgrade remix_cli, then review with --diff.
```
...
into its row and compact sheet.
