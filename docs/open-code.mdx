...
| --- | --- | --- |
| `default` | `AcmeThemeScope` | `AcmeThemeData.light()`, `.dark()`, and `AcmeThemeMode` |
...
The [downloadable tutorial apps](/tutorials/settings-screen) show the same API
...
Existing `AcmeThemeScope(data: ...)` calls remain a fixed-theme shorthand.
Fortal's direct design options and `brightness` remain supported; direct options
override the supplied theme values, and explicit `brightness` overrides mode
selection. Prefer `theme`, `darkTheme`, and `mode` in new app setup.
