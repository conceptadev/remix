...
**Required**. Accepts established `IconData` or an arbitrary icon `Widget`.
Widget descendants inherit the resolved icon style.

#### `semanticLabel` → `String?`

Optional. Supply a nonempty localized name for icon-only actions.
...
Optional. Whether to exclude icon semantics. When true, the semantics of the
icon content will be excluded. Defaults to false.
