...
Place one `RemixToastScope` above your app's `Navigator`, so every route, including dialogs, can reach it. With `MaterialApp`, put it in `builder` inside `Overlay.wrap`, which gives toasts their own overlay above every route. No `Scaffold` is required.
...
<Info>
  See the [fortalToastStyle source code](https://github.com/conceptadev/remix/blob/main/registry_source/lib/src/fortal/components/toast.dart) for all available options.
</Info>
