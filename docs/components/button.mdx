...
Optional arbitrary button content. Descendant text and icons inherit the
resolved label and icon styles. Provide either `child` or `label`.

#### `label`, `leadingIcon`, `trailingIcon`

The established text-and-icon composition. The corresponding builder fields
can replace how each resolved part is rendered.
